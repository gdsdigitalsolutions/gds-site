...
(function () {
  ...
  /* ---------- live local time (visitor's own timezone) ---------- */
  function initLiveTime() {
    var box = document.querySelector("[data-livetime]");
    if (!box || typeof Intl === "undefined" || !Intl.DateTimeFormat) return;

    var labelEl = box.querySelector("[data-livetime-label]");
    var dateEl = box.querySelector("[data-livetime-date]");
    var clockEl = box.querySelector("[data-livetime-clock]");
    ...
    if (labelEl) labelEl.textContent = (city ? city + " " : "") + "Live Time";
    ...
    function tick() {
      var now = new Date();
      var d = now.toLocaleDateString(undefined, dateOpts);
      if (d !== lastDate) { lastDate = d; if (dateEl) dateEl.textContent = d; }
      if (clockEl) {
        clockEl.textContent = now.toLocaleTimeString(undefined, timeOpts);
    ...
    tick();
    box.hidden = false;
    // align the first tick to the next whole second, then every second
